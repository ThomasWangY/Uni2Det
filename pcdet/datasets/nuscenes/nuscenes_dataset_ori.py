...
class NuScenesDataset(DatasetTemplate):
    ...
    def include_nuscenes_data(self, mode):
        ...
        for info_path in self.dataset_cfg.INFO_PATH[mode]:
            if self.oss_path is None:
                info_path = Path('/ssd2/zouzhikang/wangyubin/3DTrans/data/v1.0-trainval') / info_path # TODO wyb
        ...
        ############################### TODO wyb ###############################
        interval = {'train':1, 'test':1}
        if interval[mode] > 1:
            sampled_nusc_infos = []
            for k in range(0, len(self.infos), interval[mode]):
    ...
    def get_sweep(self, sweep_info):
        ...
        if self.oss_path is None:
            lidar_path = Path('/ssd2/zouzhikang/wangyubin/3DTrans/data/nuscenes') / sweep_info['lidar_path'] # TODO wyb
    ...
    def get_lidar_with_sweeps(self, index, max_sweeps=1):
        ...
        if self.oss_path is None:
            lidar_path = Path('/ssd2/zouzhikang/wangyubin/3DTrans/data/nuscenes') / info['lidar_path'] # TODO wyb
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        if kwargs['eval_metric'] in ['kitti', 'waymo']: # TODO wyb
    ...
    def create_groundtruth_database(self, used_classes=None, max_sweeps=10):
        ...
        database_save_path = Path('/ssd2/zouzhikang/wangyubin/3DTrans/data/v1.0-trainval') / f'gt_database_{max_sweeps}sweeps_withvelo_10' # TODO wyb
        db_info_save_path = Path('/ssd2/zouzhikang/wangyubin/3DTrans/data/v1.0-trainval') / f'nuscenes_dbinfos_{max_sweeps}sweeps_withvelo_10.pkl' # TODO wyb
        ...
        for idx in tqdm(range(len(self.infos))):
            ...
            for i in range(gt_boxes.shape[0]):
                ...
                if (used_classes is None) or gt_names[i] in used_classes:
                    db_path = Path('gt_database_10sweeps_withvelo_10') / filename  # gt_database/xxxxx.bin # TODO wyb
...
def create_nuscenes_info(version, data_path, save_path, max_sweeps=10):
    ...
    # TODO wyb
    train_nusc_infos = train_nusc_infos[::10]
    ...
    if version == 'v1.0-test':
        ...
        with open(save_path / f'nuscenes_infos_{max_sweeps}sweeps_train_10.pkl', 'wb') as f: # TODO wyb
            pickle.dump(train_nusc_infos, f)
