...
class _UniNormR(nn.Module):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True, track_running_stats=True, voxel_coord=False):
        super(_UniNormR, self).__init__()
        ...
        H, W = 376, 376
        self.kitti_1 = (kitti_range[0] - waymo_range[0])/(waymo_range[2] - waymo_range[0])
        self.kitti_2 = (kitti_range[2] - waymo_range[0])/(waymo_range[2] - waymo_range[0])
        self.kitti_3 = (kitti_range[1] - waymo_range[1])/(waymo_range[3] - waymo_range[1])
        self.kitti_4 = (kitti_range[3] - waymo_range[1])/(waymo_range[3] - waymo_range[1])
        self.nusc_1 = (nusc_range[0] - waymo_range[0])/(waymo_range[2] - waymo_range[0])
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            N, D, H, W = x.shape
            x_ = x.detach()
            batch_1 = x_[:N//2, :, int(self.kitti_3*H):int(self.kitti_4*H), int(self.kitti_1*W):int(self.kitti_2*W)].permute(1, 0, 2, 3).reshape(D, -1)
            batch_2 = x_[N//2:, :, int(self.nusc_3*H):int(self.nusc_4*H), int(self.nusc_1*W):int(self.nusc_2*W)].permute(1, 0, 2, 3).reshape(D, -1)
            batch_merge = torch.cat((batch_1, batch_2), dim=1)
            batch_mean = torch.mean(batch_merge, dim=-1)
            batch_mean = batch_mean.unsqueeze(0).unsqueeze(2).unsqueeze(3)
            batch_var = torch.var(batch_merge, dim=-1)
            batch_var = batch_var.unsqueeze(0).unsqueeze(2).unsqueeze(3)
            ...
            running_mean = self.running_mean.unsqueeze(0).unsqueeze(2).unsqueeze(3)
            running_var = self.running_var.unsqueeze(0).unsqueeze(2).unsqueeze(3)
            x_normalized = (x - running_mean) / torch.sqrt(running_var + self.eps)
        
        # Scale and shift
        scaled = self.weight.unsqueeze(0).unsqueeze(2).unsqueeze(3) * x_normalized
        shifted = scaled + self.bias.unsqueeze(0).unsqueeze(2).unsqueeze(3)
        return shifted
