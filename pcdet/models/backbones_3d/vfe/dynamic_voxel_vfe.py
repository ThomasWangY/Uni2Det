...
from .dynamic_pillar_vfe import PFNLayer, PFNLayerV1, PFNLayerV2

# TODO: point distribution correction
class DynamicVoxelVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, grid_size, point_cloud_range, **kwargs):
        ...
        self.entity_bn = self.model_cfg.ENTITY_BN
        ...
        for i in range(len(num_filters) - 1):
            ...
            if self.entity_bn:
                pfn_layers.append(
                    PFNLayerV2(in_filters, out_filters, alpha, self.use_norm, last_layer=(i >= len(num_filters) - 2))
                )
            else:
                pfn_layers.append(
                    PFNLayerV1(in_filters, out_filters, alpha, self.use_norm, last_layer=(i >= len(num_filters) - 2))
                )
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        if self.use_absolute_xyz:
            ...
            features = [point_xyz, f_cluster, f_center]
        else:
            features = [points[:, 4:], f_cluster, f_center]
        
        if self.with_distance:
            points_dist = torch.norm(points[:, 1:4], 2, dim=1, keepdim=True)  # _rel
            features.append(points_dist)
        features = torch.cat(features, dim=-1)

        for pfn in self.pfn_layers:
            features = pfn(features, unq_inv, points, len(batch_dict['db_flag'])) # 
